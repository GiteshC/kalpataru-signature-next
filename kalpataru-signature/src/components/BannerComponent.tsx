    ...
    <section className="homebannerSec projectdetBanner">
      <div className="videoBox">
        {videoUrl || bannerData?.banner_options?.[0]?.banner_video_field ? (
          ...
          <Image src={`${imageUrl ? imageUrl : bannerData?.banner_options?.[0]?.banner_image_field?.url}`} alt="" width={bannerData?.banner_options?.[0]?.banner_image_field?.width} height={bannerData?.banner_options?.[0]?.banner_image_field?.height} style={{ width: "100%", height: "auto" }} />
        )}
      </div>
      <div className="homebannerWrapper homebannerContent">
        <div className="secHeading philosopy-banner">
          ...
          <p className={`bannerTextanimation ${isPageLoad ? "bannerTextanimationnew" : ""}`}>
            <span>{para}</span>
          </p>
          {bannerSubHeading && (
      ...
      <div className={`scrollText ${isPageLoad ? "scrollTextnew" : ""}`}>
        <h3>{mainHeading.trim().toLocaleUpperCase() === "SIGNATURE RESIDENCES" ? "Explore projects" : "Scroll to Explore"}</h3>
      </div>
    </section>
  );
};
